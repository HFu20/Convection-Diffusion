function [E2, Emax, m_err ,wtime ] = main_Solve_u_inter_Lag_split
...
%     u_n =  (u_bar_nPlusHf5-miu_y/4*gamma*u_nPlusHf4*matrix_Delta_y_2')/ H_1y' ;
 ...
 [ kxi_plus(k) ,count(k) ] = new_fun_kxi_plus( dt, u_nPlus1, u_n, a, b , kxi, dx,dy,Mass_S_half(k),lam_n ) ;
...
Relative_masserr = Mass_err(1:end)./Mass_u(2:end) ;
figure
plot(tt(2:end),Relative_masserr,LineWidth=1.4)
legend('Relative_{err}')
xticks([T0 (T-T0)/4 (T-T0)/2  3*(T-T0)/4 (T-T0)])
ylim([-5e-15 5e-15])
...
Mass_U = zeros(nt+1,1) ;
for k =1:nt+1
  U  =  Exact_sol( x, y,  tt(k), gamma,sigma2  ) ;  
  Mass_U(k) = sum(sum(U))*dx*dy;
end

Mass_err_U = Mass_U(2:end) - Mass_U(1:end-1) ;
Relative_masserr_U = Mass_err_U(1:end)./Mass_U(2:end) ;
figure
plot(tt(2:end),Relative_masserr_U,LineWidth=1.4)
legend('Relative_{err}')
xticks([T0 (T-T0)/4 (T-T0)/2  3*(T-T0)/4 (T-T0)])
ylim([-5e-15 5e-15])
