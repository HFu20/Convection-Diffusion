function [ kxi_plus,  Gk,count ] = new_fun_kxi_plus( dt, u_nPlus1, u_n, a, b , kxi, dx,dy,Mass_S,lam_n )
...
v_minus = u_nPlus1 +  yinta_minus ; %  原文多了dt
Gminus_1 = 0 ; %表示( k-1 )层的四个求和项（只需计算一遍）
Gminus_2 = 0 ;
Gminus_3 = 0 ;
Gminus_4 = 0;

for i = 1: length( u_nPlus1( :,1 ) )
    for j = 1: length( u_nPlus1( 1,: ) )

        if  v_minus( i, j ) <= a
            Gminus_3 = Gminus_3+ a* w ;
        elseif  v_minus( i, j ) >= b
            Gminus_2 = Gminus_2 +b* w;
        elseif v_minus( i, j )<b &&  v_minus(i,j )>a
...
Gminus = Gminus_1 + Gminus_2 + Gminus_3 - Gminus_4 -  Mass_S;

while abs( kxi_k - kxi_minus ) >1e-14  && count<8   %eps =  1.0e-6;
    ...
    for i = 1: length( u_nPlus1( :,1 ) )
        for j = 1: length( u_nPlus1( 1,: ) )
            ...
            elseif  v_k(i,j ) >= b
                Gk_2 = Gk_2 + b* w ;
            elseif v_k(i,j )<b &&   v_k(i,j )>a
                Gk_1 = Gk_1 + v_k(i,j )* w ;
            end
            Gk_4 = Gk_4 + u_n( i,j )* w ;  % （u^n,1）的内积
        end
    end
    Gk = Gk_1 + Gk_2 + Gk_3 - Gk_4 -  Mass_S; 
    %     迭代求解 kxi
