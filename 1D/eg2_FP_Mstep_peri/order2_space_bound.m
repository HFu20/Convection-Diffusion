...
xmin =-2*pi; xmax = 2*pi ;
T0=0;T =0.5;
D_x = [   0.008*pi, 0.016*pi , 0.032*pi, 0.064*pi , 0.128*pi];
N_x =  floor([ (xmax-xmin)/D_x(5), (xmax-xmin)/D_x(4)-4, (xmax-xmin)/D_x(3)-3, (xmax-xmin)/D_x(2)-7, (xmax-xmin)/D_x(1)-17 ] );
...
E2(1) = sqrt(sum( D_x(1) *( ( u_1( 1:1:end-1 ) -  u_5( 1: 16: end ) ).^2))); 
E2(2) = sqrt(sum( D_x(2) *( ( u_2( 1: 2: end ) - u_5( 1: 16: end ) ).^2))); 
E2(3) = sqrt(sum( D_x(3) *( (  u_3( 1: 4: end ) -  u_5( 1: 16: end ) ).^2))); 
E2(4) = sqrt(sum( D_x(4) *( (  u_4(  1: 8: end ) - u_5( 1: 16: end ) ).^2))); 

Emax(1) = max(max(abs( u_1( 1:1:end-1 ) -  u_5( 1: 16: end ) )  ));
Emax(2) = max(max(abs( u_2( 1: 2: end ) - u_5( 1: 16: end ) ) ));
Emax(3) = max(max(abs (   u_3( 1: 4: end ) -  u_5( 1: 16: end )  )));
Emax(4) = max(max(abs(  u_4(  1: 8: end ) - u_5( 1: 16: end )  )));
